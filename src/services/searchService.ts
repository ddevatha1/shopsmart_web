 ...
 * image backfill. Framework-agnostic on purpose (no Next.js imports
 * anywhere in this file): `src/app/api/search/route.ts` is a thin
 * NextRequest/NextResponse shell around `performSearch` below, and
 * shoppingPlanOptimizer.ts (the Smart Shopping Planner) calls it directly
 * too, one call per grocery-list item. Keeping this file free of `next/server`
 * is what makes it (and anything that imports it) loadable by the plain
 * Node test runner — a route.ts file that imports `next/server` isn't.
 *
 * Ported from shopsmart_mobile's backend/src/routes/search.ts, itself
 * originally ported FROM this file's predecessor — see git history.
 */
...
/**
 * The full search pipeline (query correction, all 4 store adapters in
 * parallel, food/relevance filtering, classification/ranking, cross-store
 * image backfill) as a plain in-process function — the same logic the
 * search route's `POST` handler runs, callable directly by other
 * server-side code (the Smart Shopping Planner's optimizer, one call per
 * grocery-list item) instead of self-issuing HTTP requests to that route.
 * `noCorrect` mirrors the route's own request body flag, for callers (like
 * the planner) that have already resolved/typo-corrected their query text.
 */
